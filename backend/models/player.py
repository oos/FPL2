...
class Player:
    ...
    def from_db_row(cls, row: tuple) -> 'Player':
        ...
        return cls(
            ...
            team=row[3],
            price=row[4],
            chance_of_playing_next_round=row[5] or 100.0,
            points_per_million=row[6] or 0.0,
            total_points=row[7] or 0.0,
            form=row[8] or 0.0,
            ownership=row[9] or 0.0,
            team_id=row[10] if len(row) > 10 else None,
            gw1_points=row[11] if len(row) > 11 else 0.0,
            gw2_points=row[12] if len(row) > 12 else 0.0,
            gw3_points=row[13] if len(row) > 13 else 0.0,
            gw4_points=row[14] if len(row) > 14 else 0.0,
            gw5_points=row[15] if len(row) > 15 else 0.0,
            gw6_points=row[16] if len(row) > 16 else 0.0,
            gw7_points=row[17] if len(row) > 17 else 0.0,
            gw8_points=row[18] if len(row) > 18 else 0.0,
            gw9_points=row[19] if len(row) > 19 else 0.0
        )
